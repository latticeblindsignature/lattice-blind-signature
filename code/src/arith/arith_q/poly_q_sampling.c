...
static void poly_q_uniform(poly_q pout, const uint8_t seed[SEED_BYTES], uint32_t domain_separator, size_t i, size_t j) {
  ...
  off = 0;
  while (cnt < PARAM_N) {
#if PARAM_Q_BITLEN > 24
#error "PARAM_Q_BITLEN too big for uniform sampling."
#else
    // idea: take 3 bytes, potentially ignore the MSBs, perform rejection sampling
    if (bytecnt < 3) {
      for (k = 0; k < bytecnt; k++) {
        output[k] = output[off++];
      }
    ...
    uint64_t tmp3byte = output[off] | ((uint64_t)output[off+1] << 8) | ((uint64_t)output[off+2] << 16);
    uint64_t tmp = tmp3byte & ((1<<PARAM_Q_BITLEN)-1);
...
/*************************************************
* Name:        poly_q_vec_d_sample_r1
*
* Description: Sample a uniform polynomial vector of size PARAM_D 
*              with deterministically from a seed, with coefficients
*              in [-2*PARAM_B1, 2*PARAM_B1).
* 
* Arguments:   - poly_q_vec_d vec: output uniform polynomial vector [-2.b1,2.b1) (initialized)
*              - const uint8_t *seed: pointer to byte array containing seed (allocated SEED_BYTES bytes)
*              - uint32_t domain_separator: domain separator for XOF
*              - uint8_t offset: offset domain separator for XOF
**************************************************/
void poly_q_vec_d_sample_r1(poly_q_vec_d vec, const uint8_t seed[SEED_BYTES], uint32_t domain_separator, uint8_t offset) {
#if (PARAM_N%64) != 0
#error "PARAM_N must be divisible by 64"
#endif
  uint8_t output[(PARAM_B1_LOG+2)*PARAM_D*PARAM_N/8];
  keccak_state state;
  size_t i,j,k,off;
  coeff_q c;
  ...
  shake256_absorb(&state, seed, SEED_BYTES);
  shake256_absorb(&state, (const uint8_t*)&domain_separator, sizeof(uint32_t));
  shake256_absorb(&state, (const uint8_t*) &offset, sizeof(offset));
  shake256_finalize(&state);
  shake256_squeeze(output, (PARAM_B1_LOG+2)*PARAM_D*PARAM_N/8, &state);

  // idea: PARAM_B1_LOG+2 = 11 bits. Take 11 bytes (88 bits), and fill 8 polynomial coefficients at a time
  off = 0;
  for (i = 0; i < PARAM_D; i++) {
    for (j = 0; j < PARAM_N/8; j++) {
      uint64_t tmp6byte = output[off] | ((uint64_t)output[off+1] << 8) | ((uint64_t)output[off+2] << 16) | ((uint64_t)output[off+3] << 24) | ((uint64_t)output[off+4] << 32) | ((uint64_t)output[off+5] << 40);
      for (k = 0; k < 4; k++) {
        c = (coeff_q)(tmp6byte & ((PARAM_B1 << 2) - 1)) - (PARAM_B1 << 1);
        poly_q_set_coeff(vec->entries[i], 8*j+k, c);
        tmp6byte >>= (PARAM_B1_LOG+2);
      }
      tmp6byte |= (output[off+6] | ((uint64_t)output[off+7] << 8) | ((uint64_t)output[off+8] << 16) | ((uint64_t)output[off+9] << 24) | ((uint64_t)output[off+10] << 32)) << 4;
      for (k = 4; k < 8; k++) {
        c = (coeff_q)(tmp6byte & ((PARAM_B1 << 2) - 1)) - (PARAM_B1 << 1);
        poly_q_set_coeff(vec->entries[i], 8*j+k, c);
        tmp6byte >>= (PARAM_B1_LOG+2);
...
/*************************************************
* Name:        poly_q_vec_d_uniform_b2
*
* Description: Sample a uniform polynomial vector of size PARAM_D 
*              with deterministically from a seed, with coefficients
*              in [-PARAM_B2, PARAM_B2)
* 
* Arguments:   - poly_q_vec_d vec: output uniform polynomial vector (initialized)
*              - const uint8_t *seed: pointer to byte array containing seed (allocated SEED_BYTES bytes)
*              - uint32_t domain_separator: domain separator for XOF
*              - uint8_t offset: offset domain separator for XOF
**************************************************/
void poly_q_vec_d_uniform_b2(poly_q_vec_d vec, const uint8_t seed[SEED_BYTES], uint32_t domain_separator, uint8_t offset) {
#if (PARAM_N%64) != 0
#error "PARAM_N must be divisible by 64"
#endif
  uint8_t output[(PARAM_B2_LOG+1)*PARAM_D*PARAM_N/8];
  keccak_state state;
  size_t i,j,off;
  coeff_q c;
  ...
  shake256_absorb(&state, (const uint8_t*) &offset, sizeof(offset));
  shake256_finalize(&state);
  shake256_squeeze(output, (PARAM_B2_LOG+1)*PARAM_D*PARAM_N/8, &state);

  // idea: PARAM_B2_LOG+1 = 4 bits. Take 1 byte, and fill 2 polynomial coefficients at a time
  off = 0;
  for (i = 0; i < PARAM_D; i++) {
    for (j = 0; j < PARAM_N/2; j++) {
      uint64_t tmp1byte = output[off];
      c = (coeff_q)(tmp1byte & ((PARAM_B2 << 1) - 1)) - PARAM_B2;
      poly_q_set_coeff(vec->entries[i], 2*j, c);
      tmp1byte >>= (PARAM_B2_LOG+1);
      c = (coeff_q)(tmp1byte & ((PARAM_B2 << 1) - 1)) - PARAM_B2;
      poly_q_set_coeff(vec->entries[i], 2*j+1, c);
      off += 1;
    }
  }
}
...
/*************************************************
* Name:        poly_q_vec_k_uniform_b2
*
* Description: Sample a uniform polynomial vector of size PARAM_K 
*              with deterministically from a seed, with coefficients
*              in [-PARAM_B2, PARAM_B2)
* 
* Arguments:   - poly_q_vec_d vec: output uniform polynomial vector (initialized)
...
*              - uint32_t domain_separator: domain separator for XOF
**************************************************/
void poly_q_vec_k_uniform_b2(poly_q_vec_k vec, const uint8_t seed[SEED_BYTES], uint32_t domain_separator) {
#if (PARAM_N%64) != 0
#error "PARAM_N must be divisible by 64"
#endif
  uint8_t output[(PARAM_B2_LOG+1)*PARAM_K*PARAM_N/8];
  keccak_state state;
  size_t i,j,off;
  coeff_q c;
  ...
  shake256_absorb(&state, (const uint8_t*)&domain_separator, sizeof(uint32_t));
  shake256_finalize(&state);
  shake256_squeeze(output, (PARAM_B2_LOG+1)*PARAM_K*PARAM_N/8, &state);
...
static const double klein_widths[PARAM_K] = {
  3.41659824368812259365L,
  3.41663929154978873726L,
  3.45785158494839706123L
};
...
static const double neg_scaled_gadget_gso[PARAM_K][PARAM_K] = {
  {-0.00490184299685224745L,  -0.00002402864212755870L, -0.00000011920967324495L},
  {0.00002402864214143259L, -0.00490184299402197498L, -0.00002431877334199855L},
  {0.00000000000000000000L, 0.00002402921951707542L,  -0.00496102976176781250L}
};
