...
* Name:        decompose_b1
*
* Description: Decompose c as low_c + PARAM_B1*high_c with low_c
*              in [-PARAM_B1, PARAM_B1)
*
* Arguments:   - coeff_q *low_c: coefficient to host low part of c
*              - const coeff_q c: coefficient to be decomposed
* 
* Returns high_c the high part of c
**************************************************/
static coeff_q decompose_b1(coeff_q *low_c, const coeff_q c) {
  coeff_q high_c;
  high_c = 2 * (c >> (PARAM_B1_LOG+1)) + 1; // high_c = ((c >> (PARAM_B1_LOG+1)) << 1) + 1; // left shift of signed int cause runtime errors
  *low_c = c - PARAM_B1 * high_c;           // *low_c = c - (high_c << PARAM_B1_LOG); 
  return high_c;
}
...
* Name:        decompose_b2
*
* Description: Decompose c as low_c + PARAM_B2*high_c with low_c
...
* Returns high_c the high part of c
**************************************************/
static coeff_q decompose_b2(coeff_q *low_c, const coeff_q c) {
  coeff_q high_c;
  high_c = 2 * (c >> (PARAM_B2_LOG+1)) + 1; // high_c = ((c >> (PARAM_B2_LOG+1)) << 1) + 1; // left shift of signed int cause runtime errors
  *low_c = c - PARAM_B2 * high_c;           // *low_c = c - (high_c << PARAM_B2_LOG); 
  return high_c;
}
...
/*************************************************
* Name:        poly_q_decompose_b1
*
* Description: Decompose coefficients of input polynomial into signed high order
*              and low order bits with decomposition base PARAM_B1 (power-of-two)
*
* Arguments:   - poly_q high: polynomial to host the high order decomposition (initialized)
*              - poly_q low: polynomial to host the low order decomposition (initialized)
*              - const poly_q arg: polynomial to be decomposed
**************************************************/
void poly_q_decompose_b1(poly_q high, poly_q low, const poly_q arg) {
  coeff_q c, low_c;
  for (size_t i = 0; i < PARAM_N; i++) {
    c = poly_q_get_coeff_centered(arg, i); // must be on centered representation
    poly_q_set_coeff(high, i, decompose_b1(&low_c, c));
    poly_q_set_coeff(low, i, low_c);
  }
}

/*************************************************
* Name:        poly_q_decompose_b2
*
* Description: Decompose coefficients of input polynomial into signed high order
*              and low order bits with decomposition base PARAM_B2 (power-of-two)
*
* Arguments:   - poly_q high: polynomial to host the high order decomposition (initialized)
*              - poly_q low: polynomial to host the low order decomposition (initialized)
*              - const poly_q arg: polynomial to be decomposed
**************************************************/
void poly_q_decompose_b2(poly_q high, poly_q low, const poly_q arg) {
  coeff_q c, low_c;
  for (size_t i = 0; i < PARAM_N; i++) {
    c = poly_q_get_coeff_centered(arg, i); // must be on centered representation
    poly_q_set_coeff(high, i, decompose_b2(&low_c, c));
    poly_q_set_coeff(low, i, low_c);
  }
}
...
* Name:        poly_q_minmax
*
* Description: Compute the interval [min, max] in which lie the coefficients
*
* Arguments:   - int64_t *min: pointer to the minimum of all coefficients
*              - const poly_q arg: the polynomial
*
* Returns the maximum of all coefficients
**************************************************/
int64_t poly_q_minmax(int64_t *min, const poly_q arg) {
  int64_t max, c;
  max = (int64_t)poly_q_get_coeff_centered(arg, 0);
  *min = max;
  for (size_t i = 1; i < PARAM_N; i++) {
    c = (int64_t)poly_q_get_coeff_centered(arg, i);
    max = (c > max) ? c : max;
    *min = (c < *min) ? c : *min;
  }
  return max;
