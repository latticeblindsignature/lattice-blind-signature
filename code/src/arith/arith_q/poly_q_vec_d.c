...
/*************************************************
* Name:        poly_q_vec_d_decompose_b1
*
* Description: Decompose coefficients of input vector into signed high order
*              and low order bits with decomposition base PARAM_B1 (power-of-two)
*
* Arguments:   - poly_q_vec_d high: polynomial vector to host the high order decomposition (initialized)
*              - poly_q_vec_d low: polynomial vector to host the low order decomposition (initialized)
*              - const poly_q_vec_d arg: polynomial vector to be decomposed
**************************************************/
void poly_q_vec_d_decompose_b1(poly_q_vec_d high, poly_q_vec_d low, const poly_q_vec_d arg) {
  for (size_t i = 0; i < PARAM_D; ++i) {
  	poly_q_decompose_b1(high->entries[i], low->entries[i], arg->entries[i]);
  }
}

/*************************************************
* Name:        poly_q_vec_d_decompose_b2
*
* Description: Decompose coefficients of input vector into signed high order
...
*              - const poly_q_vec_d arg: polynomial vector to be decomposed
**************************************************/
void poly_q_vec_d_decompose_b2(poly_q_vec_d high, poly_q_vec_d low, const poly_q_vec_d arg) {
  for (size_t i = 0; i < PARAM_D; ++i) {
  	poly_q_decompose_b2(high->entries[i], low->entries[i], arg->entries[i]);
  }
}
...
*              - const poly_q_vec_d arg: the polynomial vector
*
* Returns the maximum of all coefficients
**************************************************/
int64_t poly_q_vec_d_minmax(int64_t *min, const poly_q_vec_d arg) {
  int64_t max, tmpmax, tmpmin;
  max = poly_q_minmax(min, arg->entries[0]);
  for (size_t i = 1; i < PARAM_D; i++) {
    tmpmax = poly_q_minmax(&tmpmin, arg->entries[i]);
