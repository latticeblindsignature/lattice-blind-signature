...
void arith_teardown(void) {
  ...
  poly_p_vec_d_teardown();
  poly_p_vec_m_teardown();
  poly_p_mat_d_m_teardown();
  ...
  poly_qiss_vec_m1_teardown();
  poly_qiss_vec_m2_d_teardown();
  poly_qiss_vec_l_teardown();
  poly_qiss_vec_256_teardown();
  poly_qiss_vec_256_l_teardown();
  poly_qiss_mat_k_k_teardown();
  poly_qiss_mat_256l_m2_d_teardown();
  poly_qiss_mat_d_m1_teardown();
  poly_qiss_mat_d_m2_d_teardown();

  poly_qshow_vec_d_teardown();
  poly_qshow_vec_m1_teardown();
  poly_qshow_vec_m2_d_teardown();
  ...
  poly_qshow_mat_d_m2_d_teardown();
  poly_qshow_mat_256l_m2_d_teardown();
...
/*************************************************
* Name:        poly_q_qiss_subring_embed_vec_k
*
...
**************************************************/
void poly_q_qiss_subring_embed_vec_k(poly_qiss_vec_k res, const poly_q arg, const int64_t fac) {
  for (size_t i = 0; i < PARAM_K_ISS; i++) {
    for (size_t j = 0; j < PARAM_N_ISS; j++) {
      poly_qiss_set_coeff(res->entries[i], j, poly_q_get_coeff_centered(arg, i + j*PARAM_K_ISS));
    }
  }
  poly_qiss_vec_k_mul_scalar(res, res, fac);
}

/*************************************************
* Name:        poly_p_qiss_subring_embed_vec_k
*
* Description: Subring embedding and lifting mod PARAM_Q_ISS. Multiplication by
*              scalar during lifting (paper notation: \theta)
* 
* Arguments:   - poly_qiss_vec_k res: polynomial vector result of theta(arg) modulo PARAM_Q_ISS (initialized)
*              - const poly_p arg: polynomial to be embedded
**************************************************/
void poly_p_qiss_subring_embed_vec_k(poly_qiss_vec_k res, const poly_p arg) {
  size_t i,j;
  coeff_p c_p;
  for (i = 0; i < PARAM_K_ISS; i++) {
    for (j = 0; j < PARAM_N_ISS; j++) {
      c_p = poly_p_get_coeff_centered(arg, i + j*PARAM_K_ISS);
      poly_qiss_set_coeff(res->entries[i], j, (coeff_qiss)c_p);
    }
  }
}

/*************************************************
* Name:        poly_q_qiss_subring_embed_mat_k_k
*
...
*              - const int64_t fac: scalar factor during lifting (1 or PARAM_Q1_ISS)
**************************************************/
void poly_q_qiss_subring_embed_mat_k_k(poly_qiss_mat_k_k res, const poly_q arg, const int64_t fac) {
  size_t i, j;
  ...
  poly_q_qiss_subring_embed_vec_k(tmp_vec, arg, fac);
...
/*************************************************
* Name:        poly_p_qiss_subring_embed_mat_k_k
*
* Description: Subring multiplication matrix embedding and lifting mod PARAM_Q_ISS. Multiplication by
*              scalar during lifting (paper notation: M_{\theta})
* 
* Arguments:   - poly_qiss_mat_k_k res: polynomial matrix result of M_theta(arg) modulo PARAM_Q_ISS (initialized)
*              - const poly_q arg: polynomial to be embedded
**************************************************/
void poly_p_qiss_subring_embed_mat_k_k(poly_qiss_mat_k_k res, const poly_p arg) {
  size_t i, j;
  poly_qiss_vec_k tmp_vec, tmp_vec_x;
  ...
  // subring embedding vector
  poly_p_qiss_subring_embed_vec_k(tmp_vec, arg);
...
*              - const poly_q arg: polynomial to be embedded
*              - const int64_t fac: scalar factor during lifting 
**************************************************/
void poly_qshow_subring_embed_vec_k(poly_qshow_vec_k res, const poly_q arg, const int64_t fac) {
  for (size_t i = 0; i < PARAM_K_SHOW; i++) {
    for (size_t j = 0; j < PARAM_N_SHOW; j++) {
      poly_qshow_set_coeff(res->entries[i], j, poly_q_get_coeff_centered(arg, i + j*PARAM_K_SHOW));
    }
  }
  poly_qshow_vec_k_mul_scalar(res, res, fac);
}
