...
#if PARAM_Q_SHOW_BITLEN > 52
#error "PARAM_Q_SHOW_BITLEN too big for uniform sampling."
#else
    // idea: take 13 bytes, ignore MSBs
    if (bytecnt < 13) {
      for (k = 0; k < bytecnt; k++) {
        output[k] = output[off++];
      }
    ...
    uint64_t tmp = tmp7byte & ((1UL<<PARAM_Q_SHOW_BITLEN)-1);
    if (tmp < PARAM_Q_SHOW) {
    ...
    tmp7byte >>= PARAM_Q_SHOW_BITLEN;
    tmp7byte |= (output[off+7] | ((uint64_t)output[off+8] << 8) | ((uint64_t)output[off+9] << 16) | ((uint64_t)output[off+10] << 24) | ((uint64_t)output[off+11] << 32) | ((uint64_t)output[off+12] << 40)) << 5;
    tmp = tmp7byte & ((1UL<<PARAM_Q_SHOW_BITLEN)-1);
    if (tmp < PARAM_Q_SHOW) {
      poly_qshow_set_coeff(pout, cnt++, tmp);
    }
    off += 13;
    bytecnt -= 13;
#if PARAM_Q_SHOW_BITLEN < 49
...
void vec_qshow_uniform(coeff_qshow out[PARAM_ARP_SHOW + 6], const uint8_t *buf, const uint32_t domain_separator, const uint32_t counter, size_t buflen) {
  ...
  cnt = 0;
  off = 0;
  while (cnt < PARAM_ARP_SHOW + 4 + PARAM_N_SHOW - 1) {
#if PARAM_Q_SHOW_BITLEN > 52
#error "PARAM_Q_SHOW_BITLEN too big for uniform sampling."
#else
    // idea: take 13 bytes, ignore MSBs
    ...
    if (cnt >= PARAM_ARP_SHOW + 4 + PARAM_N_SHOW - 1) {
      break;
    }
    tmp7byte >>= PARAM_Q_SHOW_BITLEN;
    tmp7byte |= (output[off+7] | ((uint64_t)output[off+8] << 8) | ((uint64_t)output[off+9] << 16) | ((uint64_t)output[off+10] << 24) | ((uint64_t)output[off+11] << 32) | ((uint64_t)output[off+12] << 40)) << 5;
...
/*************************************************
* Name:        poly_qshow_uniform_but_zero_half
*
* Description: Sample a uniformly random polynomial modulo
...
*              - uint32_t domain_separator: domain separator for XOF
**************************************************/
void poly_qshow_uniform_but_zero_half(poly_qshow out, const uint8_t seed[SEED_BYTES], uint32_t kappa, uint32_t domain_separator) {
  // TODO implement dedicated function?
  poly_qshow_uniform(out, seed, domain_separator, kappa, 0);
  poly_qshow_set_coeff(out, 0, 0);
  poly_qshow_set_coeff(out, PARAM_N_SHOW/2, 0);
}
...
/*************************************************
* Name:        poly_q_mat_d_m2_d_uniform
*
* Description: Sample a uniformly random polynomial matrix of
*              size PARAM_D_SHOW x PARAM_M2_D_SHOW modulo PARAM_Q_SHOW 
*              deterministically from a seed.
* 
...
*              - uint32_t domain_separator: domain separator for XOF
**************************************************/
void poly_qshow_mat_d_m2_d_uniform(poly_qshow_mat_d_m2_d mat, const uint8_t seed[SEED_BYTES], uint32_t domain_separator) {
  size_t i,j;
  for (i = 0; i < PARAM_D_SHOW; i++) {
    for (j = 0; j < PARAM_M2_D_SHOW; j++) {
      poly_qshow_uniform(mat->rows[i]->entries[j], seed, domain_separator, i, j);
    }
  }
}
...
/*************************************************
* Name:        poly_q_mat_256l_m2_d_uniform
*
* Description: Sample a uniformly random polynomial matrix of
*              size PARAM_ARP_DIV_N_L_SHOW x PARAM_M2_D_SHOW modulo PARAM_Q_SHOW 
*              deterministically from a seed.
* 
* Arguments:   - poly_qshow_mat_256l_m2_d mat: output uniform polynomial matrix (initialized)
*              - const uint8_t *seed: pointer to byte array containing seed (allocated SEED_BYTES bytes)
*              - uint32_t domain_separator: domain separator for XOF
**************************************************/
void poly_qshow_mat_256l_m2_d_uniform(poly_qshow_mat_256l_m2_d mat, const uint8_t seed[SEED_BYTES], uint32_t domain_separator) {
  size_t i,j;
...
/*************************************************
* Name:        poly_qshow_vec_m2_d_uniform
*
* Description: Sample a uniformly random polynomial vector of
*              size PARAM_M2_D_SHOW modulo PARAM_Q_SHOW 
*              deterministically from a seed.
* 
* Arguments:   - poly_qshow_vec_m2_d vec: output uniform polynomial matrix (initialized)
*              - const uint8_t *seed: pointer to byte array containing seed (allocated SEED_BYTES bytes)
*              - uint32_t domain_separator: domain separator for XOF
**************************************************/
void poly_qshow_vec_m2_d_uniform(poly_qshow_vec_m2_d vec, const uint8_t seed[SEED_BYTES], uint32_t domain_separator) {
  size_t i;
...
/*************************************************
* Name:        poly_qshow_vec_l_1_uniform
*
* Description: Sample a uniformly random polynomial vector of
*              size PARAM_L_SHOW modulo PARAM_Q_SHOW and extra polynomial
*              deterministically from an input buffer.
* 
* Arguments:   - poly_qshow_vec_l vec: output uniform polynomial vector (initialized)
*              - poly_qshow pout: output uniform polynomial (initialized)
*              - const uint8_t *buf: pointer to byte buffer containing the XOF input
*              - uint32_t domain_separator: domain separator for XOF
*              - size_t buflen: length of the input buffer
**************************************************/
void poly_qshow_vec_l_1_uniform(poly_qshow_vec_l vec, poly_qshow pout, const uint8_t *buf, uint32_t domain_separator, size_t buflen) {
  size_t i;
  ...
  poly_qshow_uniform(pout, lazybuf, domain_separator, PARAM_L_SHOW, 0);
}
...
/*************************************************
* Name:        poly_qshow_vec_m2_d_binomial
*
* Description: Sample a centered binomial polynomial vector of
*              size PARAM_M2_D_SHOW with binomial parameter 1 
*              deterministically from a buffer.
* 
* Arguments:   - poly_qshow_vec_m2_d res_1: first output binomial polynomial vector (initialized)
*              - poly_qshow_vec_d res_2: second output binomial polynomial vector (initialized)
...
*              - const uint32_t domain_separator: domain separator for XOF
**************************************************/
void poly_qshow_vec_m2_d_binomial(poly_qshow_vec_m2_d res_1, poly_qshow_vec_d res_2, const uint8_t buf[SEED_BYTES], const uint32_t cnt, const uint32_t domain_separator) {
#if (PARAM_N_SHOW%64) != 0
#error "PARAM_N_SHOW must be divisible by 64"
#endif
  uint64_t output[2*(PARAM_M2_D_SHOW + PARAM_D_SHOW)*PARAM_N_SHOW/64]; // 2 bits per coefficient
  uint64_t coef_lsb[PARAM_N_SHOW/64];
  uint64_t coef_sign[PARAM_N_SHOW/64];
  keccak_state state;
  size_t k,l;
  ...
  shake256_finalize(&state);
  shake256_squeeze((uint8_t*)output, 2*(PARAM_M2_D_SHOW + PARAM_D_SHOW)*PARAM_N_SHOW/8, &state);
  for (k = 0; k < PARAM_M2_D_SHOW; k++) {
    ...
    for (l = 0; l < PARAM_N_SHOW; l++) {
      poly_qshow_set_coeff(res_1->entries[k], l, (int32_t)((coef_lsb[l/64] >> (l%64))&1) + (int32_t)(((coef_sign[l/64] >> ((l%64))) << 1)&2) - 1);
      // we have for sign||lsb either 00 (->-1) or 01 (->0) or 10 (->1), so we reconstruct this and subtract one
    }
  }
  for (k = 0; k < PARAM_D_SHOW; k++) {
    for (l = 0; l < PARAM_N_SHOW/64; l++) {
      coef_lsb[l]  = output[2*PARAM_N_SHOW/64*PARAM_M2_D_SHOW  +  2*l+2*PARAM_N_SHOW/64*k] ^ output[2*PARAM_N_SHOW/64*PARAM_M2_D_SHOW  +  2*l+1+2*PARAM_N_SHOW/64*k];
      coef_sign[l] = output[2*PARAM_N_SHOW/64*PARAM_M2_D_SHOW  +  2*l+2*PARAM_N_SHOW/64*k] & output[2*PARAM_N_SHOW/64*PARAM_M2_D_SHOW  +  2*l+1+2*PARAM_N_SHOW/64*k];
    }
    for (l = 0; l < PARAM_N_SHOW; l++) {
      poly_qshow_set_coeff(res_2->entries[k], l, (int32_t)((coef_lsb[l/64] >> (l%64))&1) + (int32_t)(((coef_sign[l/64] >> ((l%64))) << 1)&2) - 1);
      // we have for sign||lsb either 00 (->-1) or 01 (->0) or 10 (->1), so we reconstruct this and subtract one
    }
  }
}
...
/*************************************************
* Name:        poly_qshow_vec_m2_d_sample_gaussian_s2
*
* Description: Sample a polynomial vector with PARAM_M2_SHOW entries
*              from the centered spherical Gaussian with parameter
*              PARAM_S2_SHOW
* 
* Arguments:   - poly_qshow_vec_m2_d res_1: first polynomial to host the Gaussian sample
*              - poly_qshow_vec_d res_2: second polynomial to host the Gaussian sample
**************************************************/
void poly_qshow_vec_m2_d_sample_gaussian_s2(poly_qshow_vec_m2_d res_1, poly_qshow_vec_d res_2) {
  for (size_t i = 0; i < PARAM_M2_D_SHOW; i++) {
    for (size_t j = 0; j < PARAM_N_SHOW; j++) {
      poly_qshow_set_coeff(res_1->entries[i], j, SampleZ(0, PARAM_S2_SHOW));
    }
  }
  for (size_t i = 0; i < PARAM_D_SHOW; i++) {
    for (size_t j = 0; j < PARAM_N_SHOW; j++) {
      poly_qshow_set_coeff(res_2->entries[i], j, SampleZ(0, PARAM_S2_SHOW));
    }
  }
}
