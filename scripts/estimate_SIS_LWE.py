...
import sys, os
estimator_path = os.path.realpath('') + '/lattice_estimator'
...
def core_svp_cost(b: int, it=1, cost_model='sieving'):
    ...
    - output: 
        (flt)   csec        -- Classical bit security
        (flt)   qsec        -- Quantum bit security
    """
    if cost_model == 'sieving':
        csec, qsec = 0.292 * b + log2(it), 0.257 * b + log2(it)
    elif cost_model == 'realistic_sieving':
        csec, qsec = 0.292 * b + log2(it) + 16.4, 0.257 * b + log2(it) + 16.4
    elif cost_model == 'pessimistic':
        csec, qsec = 0.2075 * b + log2(it), 0.2075 * b + log2(it)
    elif cost_model == 'minspace':
        csec, qsec = 0.368 * b + log2(it), 0.2975 * b + log2(it)
    elif cost_model == 'enumeration_APS':
        exponent = 0.187 * b * log2(b) - 1.019 * b + 16.1
        csec, qsec = exponent + log2(it), exponent / 2 + log2(it)
...
def estimate_SIS(n: int, m: int, q: int, beta: float, betainf=-1, cost_model='sieving', it=1):
    ...
    - input: 
        ...
        (int)   q           -- Modulus
        (flt)   beta        -- Euclidean norm bound of the solution
        (flt)   betainf     -- Infinity norm bound of the solution
        (str)   cost_model  -- SVP oracle modeling [Optional]
        (int)   it          -- Number of calls to SVP oracle [Optional]
    - output: 
...
def estimate_LWE(n: int, q: int, Xs, Xe, m: int, it=1, cost_model='sieving', rough=True):
    """
    ...
    if rough:
        ...
        results = LWE.estimate(LWE.Parameters(n=n, q=q, Xs=Xs, Xe=Xe, m=m), deny_list=("bkw","arora-gb"))
